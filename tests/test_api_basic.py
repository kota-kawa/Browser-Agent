...
def controller(monkeypatch):
	...
	monkeypatch.setattr(
		api_chat, '_RUNTIME_SLOT_GUARD', type('G', (), {'acquire': lambda self: True, 'release': lambda self: None})()
	)
...
def test_api_chat_rejects_when_rate_limited(controller, monkeypatch):
	monkeypatch.setattr(
		api_chat, 'ip_rate_limit_guard', lambda _request: api_chat.JSONResponse({'error': 'limited'}, status_code=429)
	)
